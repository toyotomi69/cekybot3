	...
	try {
		...
		if (username.toLocaleLowerCase() === 'streamelements') {
			if (message.includes(' dono ')) {
				...
				client.say(
					channel,
					`Vojta právě dostal ${amountInGas.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} ${litr..toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} benzínu :) dojel by tedy ${km}km`
				)
			}
		}
